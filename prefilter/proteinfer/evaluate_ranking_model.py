...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        for family_class_code, list_of_scores in family_id_to_score_and_label.items():
            ...
            decoys_and_scores = np.concatenate((list_of_scores, decoys), axis=0)
            print(decoys_and_scores.shape)

            for threshold in range(10, 100, 2)[::-1]:
                threshold = threshold / 100
                thresholded_preds = list_of_scores.copy()
                thresholded_preds[thresholded_preds >= threshold] = 1
                thresholded_preds[thresholded_preds < threshold] = 0
                num_true_positives_above_threshold = np.count_nonzero(thresholded_preds[:, family_class_code])
                num_false_positives_above_threshold = np.count_nonzero(thresholded_preds) - num_true_positives_above_threshold
                threshold_to_true_positives[threshold] += num_true_positives_above_threshold
                threshold_to_false_positives[threshold] += num_false_positives_above_threshold
