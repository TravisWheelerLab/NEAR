...
fn filter_scores_inner() -> Result<Vec<HashMap<String, f64>>, std::io::Error> {
    ...
    let indices_array_list =
        read_hdf5_to_vec_usize("/xdisk/twheeler/daphnedemekas/new-indices.h5")?;
    let unrolled_names = read_names("/xdisk/twheeler/daphnedemekas/prefilter/target_names.txt")?;
