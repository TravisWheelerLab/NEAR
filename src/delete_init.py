...
def evaluate_multiprocessing(_config):
    ...
    query_sequence_chunks = [
        (dict(itertools.islice(query_sequences.items(), i, i + q_chunk_size)), model)
    ...
    pool.terminate()
    evaluator.evaluate_multiprocessing(
        query_names, query_embeddings, target_names, target_embeddings, lengths
    )
