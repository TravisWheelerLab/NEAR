...
class SequenceVAE(pl.LightningModule):
    ...
    def _setup_layers(self):
        ...
        self.embed = nn.Conv1d(
            in_channels=27, out_channels=self.res_block_n_filters, kernel_size=1
        )
        ...
        for i in range(self.n_res_blocks):
            ...
            if (i + 1) % 3 == 0:
                layer_list.append(
                    torch.nn.AvgPool1d(self.pool_factors[i // 3], padding=1)
                )
    ...
    def forward(self, x):
        one_hot = torch.nn.functional.one_hot(x, num_classes=27).transpose(-1, -2)
        y = self.embed(one_hot.float())
        x = self.embedding_trunk(y)
        sigma2 = self.sigma_mlp(x)
        mu = self.mu_mlp(x)
        sample = self.reparameterize(sigma2, mu)
        reconstructed = self.upsampler(sample)
        return sample, reconstructed, one_hot
    ...
    def _shared_step(self, batch):
        ...
        sampled, reconstructed, targets = self.forward(features)
        reconstructed = reconstructed[:, :, 8:-8]
        # now I'm reconstructing the one-hots.
        # maybe this wont work either.
        loss = self.kl + ((targets - torch.sigmoid(reconstructed)) ** 2).sum()

        e1, e2 = torch.split(
            reconstructed.transpose(-1, -2), reconstructed.shape[0] // 2, dim=0
        )
