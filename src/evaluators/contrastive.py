...
def filter_scores(cores_array, indices_array, unrolled_names):
...
def search_multicore(query_embedding: torch.Tensor, index, unrolled_names) -> List[Tuple[str, float]]:
        ...
        scores_array, indices_array = index.search(query_embedding.contiguous(), k=500)
...
class ContrastiveEvaluator(UniRefEvaluator):
    ...
    def compute_embedding(self, sequence: str) -> torch.Tensor:
        """Encodes the input sequence as a tensor and then passes
        through the model forward function to get the embedding tensor"""

        return (
            self.model_class(encode_string_sequence(sequence).unsqueeze(0).to(self.model_device))
            .squeeze()
            .T
        )
