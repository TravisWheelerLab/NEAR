...
def plot_roc_curve(
    ...
    for i, evalue_threshold in enumerate(evalue_thresholds):
        ...
        for idx, line in enumerate(lines):
            ...
            if recall is not None and filtration is not None and (1 - filtration) > 0.8:
                if idx % 1000 == 0:
                    print(
                        f"num_Ps: {num_positives},  num_Ds: {num_decoys},  recall: {100*recall:.3f}, filtration: {100*(1-filtration):.3f}"
                    )
    ...
    ax.plot([80, 100], [100, 0], "k--", linewidth=2)
...
def write_datafile(pairs, hmmerhits, evalue_thresholds=[1e-10, 1e-1, 1, 10], filename="data.txt"):
    ...
    for pair in pairs:
        ...
        datafile.write(
            f"{query}          {target}          {evalue}          {classname1}          {classname2}          {classname3}          {classname4}"
            + "\n"
        )
    datafile.close()
...
def generate_roc(filename, modelhitsfile, hmmerhits, figure_path):
    ...
    numpos1, numpos2, numpos3, numpos4 = write_datafile(
...
if __name__ == "__main__":
    ...
    hits_path = f"/xdisk/twheeler/daphnedemekas/prefilter-output/BlosumEvaluation/similarities"

    filename = "/xdisk/twheeler/daphnedemekas/data1_distance_sum_hits.txt"

    # similarity_hits_dict, all_similarities, all_e_values, all_biases, numhits = get_data(hits_path)

    generate_roc(filename, hits_path, all_hits_max, "ResNet1d/blosum_eval/roc.png")
